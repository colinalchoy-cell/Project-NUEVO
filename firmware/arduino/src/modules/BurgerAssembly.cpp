...
namespace BurgerAssembly {
    ...
    void init(IEncoderCounter* liftEncoder) {
        ...
        DebugLog::println(F("[BurgerAssembly] Initialized."));
    }

    void start() {
        if (sState != State::IDLE && sState != State::DONE && sState != State::ABORTED) {
            DebugLog::println(F("[BurgerAssembly] Already running."));
            return;
        }
        sStopRequested = false;
        sIngredientIndex = 0;
        sDriveAfterIndex = 0;
        sStepperPosition = 0;
        DebugLog::println(F("[BurgerAssembly] Starting sequence."));
    ...
    void task() {
        ...
        case State::HOME_STEPPER_START:
            DebugLog::println(F("[BurgerAssembly] Homing stepper..."));
            sStepperPosition = 0;
            sStepperRemaining = 0;
        ...
        case State::HOME_STEPPER_MOVING:
            ...
            if (stepperLimitTriggered()) {
                ...
                disableStepper();
                sStepperPosition = 0;
                DebugLog::println(F("[BurgerAssembly] Stepper homed."));
        ...
        case State::HOME_LIFT_START:
            DebugLog::println(F("[BurgerAssembly] Homing lift..."));
        ...
        case State::HOME_LIFT_BACKOFF:
            if (!liftLimitTriggered()) {
                ...
                sLiftEncoder->resetCount();
                DebugLog::println(F("[BurgerAssembly] Lift homed."));
                // Start first ingredient
                DebugLog::print(F("[BurgerAssembly] Picking up: "));
                DebugLog::println(kIngredients[sIngredientIndex].name);
        ...
        case State::INGREDIENT_STEP_TO_STATIC:
            if (stepperAtTarget()) {
                DebugLog::print(F("[BurgerAssembly] Done: "));
                DebugLog::println(kIngredients[sIngredientIndex].name);
                sIngredientIndex++;
                ...
                if (sIngredientIndex >= kNumIngredients) {
                    ...
                    uint32_t driveMs = (sDriveAfterIndex == 0)
                        ? kDriveBottomBunMs
                        : kDrivePattyMs;
                    DebugLog::print(F("[BurgerAssembly] Driving forward "));
                    DebugLog::println(driveMs);
                    startDrive();
                    sStateStartMs = millis();
        ...
        case State::DRIVE_FORWARD:
            if ((uint32_t)(millis() - sStateStartMs) >= (uint32_t)sStepperTarget) {
                stopDrive();
                DebugLog::println(F("[BurgerAssembly] Drive complete."));
        ...
        case State::TRANSFER_STEP_TO_DYNAMIC:
            if (stepperAtTarget()) {
                DebugLog::println(F("[BurgerAssembly] Burger assembly complete!"));
