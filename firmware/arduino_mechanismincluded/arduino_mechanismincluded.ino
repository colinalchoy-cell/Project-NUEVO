...
#include "src/modules/BurgerAssembly.h"
...
void setup() {
  ...
  // Burger assembly — pass the M3 encoder that DCMotorBringup already owns
DEBUG_SERIAL.println(F("[Setup] Initializing burger assembly..."));
BurgerAssembly::init(&encoder3);  // encoder3 = M3 lift encoder
  ...
  taskId = Scheduler::registerTask(BurgerAssembly::task, 1000 / MOTOR_UPDATE_FREQ_HZ, 6);
  if (taskId >= 0) {
      DEBUG_SERIAL.print(F("  - BurgerAssembly: Task #"));
      DEBUG_SERIAL.print(taskId);
      DEBUG_SERIAL.println(F(" @ 5ms (200Hz)"));
  }
